...
#include "rambler/memory/MemoryManager_generic.hpp"

#include <cstdlib>
#include <iostream>

#include "rambler/SafeInt3.hpp"

namespace rambler { namespace memory {

	MemoryManager * MemoryManager::default_manager() {
		static MemoryManager * _default_manager = new MemoryManager_generic;

		return _default_manager;
	}

	void * MemoryManager_generic::reserve_memory(size_t n, size_t size) {
		size_t amount;
	...
	void * MemoryManager_generic::reserve_memory(size_t amount) {
	...
	void MemoryManager_generic::release_memory(void * ptr) {
	...
	void MemoryManager_generic::share_memory(void * ptr) {
	...
	void MemoryManager_generic::free_memory(void * ptr) {
		/* NOTE: Replace lock/unlock with lock_shared/unlock_shared once shared_timed_mutex
		* becomes available.
		*/
